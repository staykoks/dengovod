...
const Register = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      setError(err.response?.data?.msg || t.regFailed);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 transition-colors relative">
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md w-full max-w-md border border-gray-100 dark:border-gray-700">
        <h2 className="text-2xl font-bold mb-6 text-center text-gray-800 dark:text-white">{t.createAccount}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            />
            {errors.name && <span className="text-red-500 text-xs">{t.nameRequired}</span>}
          </div>
          <div>
        ...
        <p className="mt-4 text-center text-sm text-gray-600 dark:text-gray-400">
          {t.alreadyHaveAccount} <Link to="/login" className="text-blue-600 dark:text-blue-400 hover:underline">{t.login}</Link>
        </p>
      </div>
    </div>
  );
